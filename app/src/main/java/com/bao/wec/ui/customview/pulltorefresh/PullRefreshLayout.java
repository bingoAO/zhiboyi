package com.bao.wec.ui.customview.pulltorefresh;
...
//        mRefreshView.layout(width / 2 - mRefreshViewWidth / 2, -mRefreshViewHeight + mCurrentOffsetTop, width / 2 + mRefreshViewHeight / 2, mCurrentOffsetTop);
    ...
    public void setOnScroll2BottomListener(OnScroll2BottomListener listener) {
        mOnScroll2BottomListener = listener;
    }
        ...
        if (getChildCount() > 0) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (child != mRefreshView){
                    mTarget = child;
                    //若是ListView进一步设置加载更多的回调
                    if(child instanceof AbsListView){
                        mListView = (AbsListView)child;
                        mListView.setOnScrollListener(onListScrollListener);
                    }else if(child instanceof RecyclerView){
                        mRecyclerView = (RecyclerView) child;
                        LogUtils.i("成功设定内部RecyclerView");
                        mRecyclerView.setOnScrollListener(onRecyclerScrollListener);
                    }
                }
            }
        }
    ...
    private AbsListView.OnScrollListener onListScrollListener = new AbsListView.OnScrollListener() {
        @Override
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            if(scrollState == SCROLL_STATE_IDLE){
                //停止滚动
                //判断是否滑动到底部
                if(mListView != null && mListView.getLastVisiblePosition() == mListView.getCount() -1 ){
                    if(mOnScroll2BottomListener != null){
                        mOnScroll2BottomListener.onBottom();
                    }
                }
            }
        }
    ...
    private RecyclerView.OnScrollListener onRecyclerScrollListener = new RecyclerView.OnScrollListener() {
        @Override
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            super.onScrollStateChanged(recyclerView, newState);
            if (newState == RecyclerView.SCROLL_STATE_IDLE){
                LogUtils.i("mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getScrollY() =" +mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getScrollY() );
                LogUtils.i("mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getY() =" +mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getY() );
                LogUtils.i("mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getTop() =" +mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getTop() );
                int[] location = new int[2];
                mRecyclerView.getChildAt(mRecyclerView.getChildCount()-1).getLocationOnScreen(location);
                LogUtils.i("location[0] = "+location[0]+ "  location[1] = "+location[1]);
                   //未成功
//                if(mOnScroll2BottomListener != null){
//                    mOnScroll2BottomListener.onBottom();
//                }
